...
$('#tipInput').keyup(async() =>{
  ...
  let tipInUsd = (tip * bnbPrice).toFixed(2);

  $('#tipAmountInEth').html(`${tip} BNB`);
...
async function getMyBalance(){
  let myBalance = await paymentGatewayInstance.methods.balance().call({from: user.attributes.ethAddress});
...
async function withdrawBtn(){
  if(!user || user.attributes.ethAddress.toLowerCase() !== address.toLowerCase()){
    ...
    if(profit > 0){
      ...
      $('#amountToWithdraw').html(` ${profit} BNB`);
    } else{
...
$("#withdraw").click(async function(){
  ...
  let profit = await getMyBalance();
  $('#withdrawText').html(`Confirm to withdraw your profits of <span class="sale-profit">${profit} BNB</span>`);
});
...
async function withdrawProfits(){
  ...
  try {
    await paymentGatewayInstance.methods.withdraw().send({from: user.attributes.ethAddress});
    ...
    $('#withdrawText').html(`Confirm to withdraw your profits of <span class="sale-profit">0 BNB</span>`);
...
function shareOptions(tokenAddress, id){
  ...
  let width = screen.width / 3;
  let height = screen.height / 3;
  let tokenPage = `https://ty-sir.github.io/client/token.html?token=${tokenAddress+id}`;
                    ...
                    <div id="card`+tokenAddress+id+`" class="card shadow-sm">
                      <div class="top-row">
                        <div class="creator-photo">
                          <a id='owner`+tokenAddress+id+`' href="https://ty-sir.github.io/client/profile.html?address=`+owner+`"><img class="owner shadow-sm" id="ownerPhoto`+tokenAddress+id+`" src="" width="40" alt="owner photo">
...
function userCard(ethAddress){
  let userCard = `<li id="userItem`+ethAddress+`" class="list-group-item d-flex justify-content-between align-items-center">
                    <a href="https://ty-sir.github.io/client/profile.html?address=`+ethAddress+`">
